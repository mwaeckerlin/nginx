...
sed -i '/autoindex/d;s,^\([ \t]*root[ \t]*\).*$,\1'${ENV_WEB_ROOT_PATH:-$WEB_ROOT_PATH}';,;/^[ \t]*root.*/a  autoindex '${AUTOINDEX}'\;\n'"${ERROR_PAGE}" /etc/nginx/conf.d/default.conf
if test -n "${PHP_PORT}"; then
    sed -i -e 's,^\([ \t]*index .*\);,\1 index.php;,' /etc/nginx/conf.d/default.conf
...
echo "**** Configuration done, starting NGINX ****"
